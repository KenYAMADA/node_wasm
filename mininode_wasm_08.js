...
function generateDummyRetValue(indent, value) {
  ...
  const block = TABs(indent) + '(i32.const ' + value + ')' + LF();
  return block;
}
...
function generateFuncRet(tree, indent, gctx, lctx) {
  const valueBlock = generate(tree[1], indent+1, gctx, lctx);
...
function generateFunCall(tree, indent, gctx, lctx) {
  ...
  if (gfunc) {
    ...
    let i = 0;
    while (tree[2 + i]) {
      argBlock = argBlock + generate(tree[2 + i], indent+1, gctx, lctx) + LF();
      i = i + 1;
    }
...
function generateCallPutn(tree, indent, gctx, lctx) {
  ...
  const valueBlock = generate(tree[2], indent+1, gctx, lctx);
  if (! valueBlock) {
...
function generateCallPuts(tree, indent, gctx, lctx) {
  ...
  //// dummy value 
  //const valueBlock = TABs(indent+1) + '(i32.const 0)';
  const valueBlock = generate(tree[2], indent+1, gctx, lctx);
...
function genereateWhile(tree, indent, gctx, lctx) {
  const conditionBlock = generate(tree[1], indent+2, gctx, lctx);

  const innerBlock = generate(tree[2], indent+3, gctx, lctx);
...
function genereateIf(tree, indent, gctx, lctx) {
  ...
  const positiveBlock = generate(tree[2], indent+2, gctx, lctx);
  ...
  // -- else --
  if (tree[3]) {
    const negativeBlock = generate(tree[3], indent+2, gctx, lctx);
...
function declareVariable(tree, indent, gctx, lctx) {
  ...
  let init = generate(tree[2], indent+1, gctx, lctx);
  if (! init) {
...
function generateBinaryOperator(tree, indent, operator, gctx, lctx) {
  const leftBlock = generate(tree[1], indent+1, gctx, lctx);
  const rightBlock = generate(tree[2], indent+1, gctx, lctx);
...
// --- compile to WAST --
const wast = compile(tree, g_ctx, l_ctx);
